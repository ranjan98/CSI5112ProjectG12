...
Future<Question> createQuestion(
  ...
  final responseGet =
      await http.get(Uri.parse('https://localhost:7067/api/Questions/'));
  var total = 0;
  if (responseGet.statusCode == 200) {
    ...
    final response = await http.post(
      Uri.parse(
        'https://localhost:7067/api/Questions/',
      ),
...
Future<Question> fetchQuestion(String id) async {
  final response =
      await http.get(Uri.parse('https://localhost:7067/api/Questions/' + id));
...
Future<Answer> createAnswer(
  ...
  final responseGet =
      await http.get(Uri.parse('https://localhost:7067/api/Answers/'));
  var total = 0;
  if (responseGet.statusCode == 200) {
    ...
    final response = await http.post(
      Uri.parse(
        'https://localhost:7067/api/Answers/',
      ),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
...
Future<Answer> fetchAnswer(String id) async {
  final response =
      await http.get(Uri.parse('https://localhost:7067/api/Answers/' + id));
